...
mod tests {
    ...
    const LIGHTS_JSON: &str = r#"{
        "1": {"name": "LivingRoom Decke", "modelid": "LCT016", "productname": "Hue color lamp",
    ...
    const SENSORS_JSON: &str = r#"{
        "10": {"name": "Bewegung Hallway", "modelid": "SML001", "productname": "Hue motion sensor",
    ...
    fn fetch_lights_returns_two() {
        ...
        let first = devs.iter().find(|d| d.name == "LivingRoom Decke").unwrap();
    ...
    fn fetch_sensors_drops_internal_pseudo_sensors() {
        ...
        assert_eq!(devs[0].name, "Bewegung Hallway");
    ...
    fn map_to_devices_preserves_firmware_relevant_data() {
        ...
        let bewegung = devices
            .iter()
            .find(|d| d.name == "Bewegung Hallway")
            .unwrap();
    ...
    fn load_config_parses_multi_bridge_yaml() {
        ...
        assert_eq!(cfg[1].name.as_deref(), Some("outdoor"));
    }
}
