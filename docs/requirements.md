...
| NFR-6 | Secrets at rest verschluesselt mit `sops` + `age`. Der age-Privatkey liegt **nur** auf dem Strato-Host (`/etc/inventory/age.key`, chmod 400, root:root) und niemals im Repo. |
...
| NFR-10 | Subdomain unter `*.bortfeld.de` o.ae., TLS via Caddy / Let's Encrypt. |
