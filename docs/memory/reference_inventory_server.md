...
- **Tailscale-Name:** `inventory`, IP `100.127.56.21`. MagicDNS loest in dieser
  Windows-Umgebung nicht auf — Verbindung ueber die IP.
- **OS:** Debian 12 (bookworm), x86_64.
- **SSH:** `root@100.127.56.21` mit dediziertem Workstation-Key
...
- Server ist initial **blank** ausgeliefert: kein git / cargo / rustc / docker.
  ...
  alt — Cargo.toml verlangt rust-version 1.75).
...
VPS). Nur das Dependency-Resolve (das `Cargo.lock` schreibt) laeuft auf der
Workstation durch.

Siehe [[reference-hasite-repo]] fuer den separaten VPS-VPS und das
SSH-Schluessel-Inventar, [[project-hasite]] fuer den Stack.
