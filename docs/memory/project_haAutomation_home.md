---
name: project-hasite-home
description: "Physische Umgebung des haAutomation-Setups — HA-Installation, CCU-Modell, Node-RED-Topologie, Z2M, VPN-Backbone."
metadata: 
  node_type: memory
  type: project
...
Smart-Home-Setup in Site, Stand 2026-05-14:
...
- **VPN-Backbone VPS → Heimnetz**: initialer Deploy laeuft auf **Tailscale** (Free-Tier). NetBird (SaaS/self-hosted) und WireGuard sind als Compose-Overlays vorbereitet, aber nicht aktiviert.
...
- **Domain**: bestehende Domain mit zu vergebender Subdomain fuer Inventory-UI. A-Record auf VPS-IP. Konkretes Subdomain-Label steht noch aus (kommt mit S14).
- **Sekundaerer Server fuer Inventory-Hosting**: VPS-VPS mit Tag `ansible-vps-stack`. Mit Heim-Netz ueber VPN verbunden. SSH-Auth zu diesem Server via deploy-key `ansible-vps-stack`, der lokal als `id_rsa` (RSA-4096) unter `C:\Users\guebr\.ssh\` liegt.
