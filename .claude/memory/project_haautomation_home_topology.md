...
description: "Physische Umgebung des ha<smarthome-location>-Setups — HA-Installation, CCU-Modell, Node-RED-Topologie, Z2M, VPN-Backbone."
metadata: 
  node_type: memory
  type: project
...
Smart-Home-Setup in <smarthome-location>, Stand 2026-05-14:
...
- **VPN-Backbone <vps-provider> → Heimnetz**: initialer Deploy laeuft auf **Tailscale** (Free-Tier). NetBird (SaaS/self-hosted) und WireGuard sind als Compose-Overlays vorbereitet, aber nicht aktiviert.
...
- **Domain**: bestehende Domain mit zu vergebender Subdomain fuer Inventory-UI. A-Record auf <vps-provider>-IP. Konkretes Subdomain-Label steht noch aus (kommt mit S14).
- **Sekundaerer Server fuer Inventory-Hosting**: VPS mit Tag `ansible-vps-stack`. Mit Heim-Netz ueber VPN verbunden. SSH-Auth zu diesem Server via deploy-key `ansible-vps-stack`, der lokal als `id_rsa` (RSA-4096) unter `<windows-userprofile>\.ssh\` liegt.
