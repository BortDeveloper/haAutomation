...
description: "Designentscheidungen fuer das ha<smarthome-location>-Projekt — Inventarisierung von HA/CCU/NR + Refactoring. Stack, Constraints, OOS."
metadata: 
  node_type: memory
  type: project
...
Projekt **ha<smarthome-location>** (Repo: BortDeveloper/ha<smarthome-location>) — Single Source of Truth fuer das Smart Home in <smarthome-location>. Zwei Hauptziele: (1) automatische Inventarisierung aller Geraete, Firmware, Software-Versionen ueber HA, Homematic CCU, Node-RED und Z2M; (2) Refactoring der ueber alle drei Systeme verteilten Automationslogik gemaess Ownership-Regeln.
...
- **Secrets:** sops + age. age-Privatkey nur auf <vps-provider>-Host unter `/etc/inventory/age.key` (chmod 400, root:root), niemals im Repo. Entschluesselung passiert zur Container-Boot-Zeit, Klartext landet auf tmpfs (`/run/inventory/…`), nie auf Disk.
