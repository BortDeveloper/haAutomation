> **Migriert aus** `stack-master/shared/audit-log/2026-05-16-ha-automation-architect.md` **am 2026-05-25** (G9-Migration, Phase C-ha; siehe `stack-master/shared/milestones/2026-05-25-general-vs-specific-migration.md`).

# Architect Audit – ha-automation – 2026-05-16

## Scope

- Projekt: `projects/ha-automation/` (Commit `52095ed feat(inventory): Paket A — strato-stack-tauglich`)
- Branch: `main`
- Phase: 1 (AUDIT), Iteration 0
- Typ laut `cockpit.yaml`: `edge`, Tech-Stack `[home-assistant, mqtt, zigbee]`
- Geprüft durch: `architect`
- Geprüft: `README.md`, `docs/{architecture,requirements,roadmap,strato-setup}.md`,
...
2. **Drei Deployment-Modi, ein Image.** `docker/docker-compose.yml` (Standalone:
   Caddy + `authgate` + austauschbarer VPN-Sidecar) und `docker/docker-compose.strato.yml`
...
4. **Komponenten und SPoF.** Das System hat einen Single-Point-of-Failure auf zwei
   Ebenen: (a) der Strato-VPS (single Host, kein HA), (b) der age-Privatkey
...
6. **Secrets-Layer „sops + age".** `inventory/secrets/.sops.yaml` enthält noch zwei
   `# TODO`-Einträge ohne Strato-Host-Pubkey. Es liegen ausschließlich `.example`-
...
7. **`:latest`-Tags in allen drei VPN-Overlays und Caddy** (`tailscale/tailscale:latest`,
   ...
   `ghcr.io/getsops/sops:latest`, `caddy:2-alpine`). Strato-Compose pinnt
   demgegenüber per Digest (`INVENTORY_IMAGE=ghcr.io/.../inventory@sha256:<digest>`).
   Inkonsistenz zwischen Standalone- und Strato-Mode.
...
8. **Authgate-Sidecar als Übergangs-SSO.** PBKDF2-HMAC-SHA256 mit 600.000 Runden
   ...
   **zweites Identity-Subsystem** parallel zum Cockpit-Ziel „Single Source of Identity:
   ...
   Authentik (laut Repo-Sprache und Strato-Setup).

9. **Container-Hardening unterschiedlich tief.** `docker-compose.strato.yml` setzt
...
13. **Dokumentation hochwertig und konsistent.** README.md, requirements.md,
    architecture.md, roadmap.md, strato-setup.md greifen sauber ineinander, der
...
| TLS überall | TLS 1.2+ | Caddy + Let's Encrypt (Standalone), Traefik + LE (Strato) | OK |
...
- **[HIGH]** **`inventory/secrets/.sops.yaml` enthält keine echten Recipients (zwei
  ...
  weil die `.enc`-Files erst gegen einen Strato-Host-Pubkey verschlüsselt werden
...
- **[MEDIUM]** **Container-Hardening nur in der Strato-Variante, nicht im Standalone-
...
- **[LOW]** **Hostname-Reuse `paperless.guebraun.org` für unzusammenhängenden
  Dienst.** Der Strato-VPS heißt im DNS nach einem anderen, nicht laufenden
...
- **[LOW]** **Kein Backup-Plan für SQLite + Compose-State.** `docs/strato-setup.md`
...
Strato-Host-Pubkey. Das ist genau der typische GitOps-Reproduzierbarkeits-
Bruch.
...
- **R3** (Adressat: `ha-automation-dev`): **`.sops.yaml`-TODOs schließen.** Echten
  Strato-Host-age-Pubkey einsetzen (ist nicht geheim) und mindestens eine
...
- **R4** (Adressat: `ha-automation-dev`): **Pinned Image-Tags für alle Compose-
  ...
  (analog `docker-compose.strato.yml`). Bei Bedarf neue Versions-Datei
...
- **R5** (Adressat: `ha-automation-dev`): **Container-Hardening auf Standalone-
  ...
  Strato-Variante. VPN-Sidecars brauchen weiter `NET_ADMIN`; das ist ok, aber
...
- **Sehr saubere Trennung zwischen App und Infrastruktur.** Die VPN-Abstraktion
  ...
  vorbildlich. Ich empfehle, das Pattern auch in `strato-stack` zu spiegeln, wo
...
- **Dokumentations-Schichtung.** README → requirements → architecture → roadmap
  → strato-setup ineinander verzahnt, jeweils mit ASCII-Diagrammen und
...
2. **„Phase 1+2+3 abgeschlossen, 52 Tests grün"-Bias.** Das Projekt deklariert
   ...
   da, Tests grün, aber Strato-Build/Test noch offen" erfasst.
